...
async function safeGetUrl(
  ...
  try {
    ...
    console.error("Failed to resolve storage file", storageId, error);
    return null;
  }
}
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    const pageWithImages = await Promise.all(
      posts.page.map(async (post) => ({
        ...post,
        featuredImageUrl: await safeGetUrl(ctx, post.featuredImage ?? null),
      }))
    );
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    const pageWithImages = await Promise.all(
      events.page.map(async (event) => ({
        ...event,
        featuredImageUrl: await safeGetUrl(ctx, event.featuredImage ?? null),
      }))
    );
    ...
    const pageWithAssets = await Promise.all(
      businesses.page.map(async (business) => {
        const imageUrls = await Promise.all(
          business.images.map((imageId) => safeGetUrl(ctx, imageId))
        );

        return {
          ...business,
          images: imageUrls.filter((url): url is string => Boolean(url)),
          logoUrl: await safeGetUrl(ctx, business.logo ?? null),
        };
      })
    );
